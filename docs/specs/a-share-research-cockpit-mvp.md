...
更新说明：若继续开发，不应只沿着本规格做普通投研驾驶舱。用户已将目标升级为「早期非共识多倍股雷达」，新版实现规格见 `docs/specs/early-multibagger-radar-redesign.md`。本文件保留为基础层和审计原则，新的 MVP 主线应优先实现弱信号收件箱、交叉验证矩阵、历史差异卡、12 个月路径和证伪地图。
